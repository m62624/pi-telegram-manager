...
- **Embedding settings are part of the memory plan.** Choose the model and its output `dim` before first use. The database stores the width, so changing `dim` prevents an old database from opening; the model name is not stored, so changing only `model` with the same `dim` can mix incompatible vectors. Use [`plugmem-cli export/import`](SETTINGS.md#memory-the-managers-long-term-memory) to migrate and recompute vectors, or move `<getAgentDir()>/extensions/pi-telegram-manager/memory/` to a backup and start fresh. Changing only the endpoint URL is safe when it serves the same model and dimension; to disable embeddings temporarily, set `enabled: false` while retaining the existing values.
- **Memory consolidation.** When a chat has been quiet for [`manager.factConsolidationQuietMs`](SETTINGS.md#manager-business-manager-and-the-telegram-side-of-mixed) (default **30 min**), the model goes back over it with its memory tools in hand — `manager_recall` to look things up, `manager_remember`, `manager_revise` to replace what the conversation overturned, `manager_forget` to drop what was wrong — and ends with `manager_done`. It **owns its own memory**: nothing is capped, evicted or expired on its behalf. A `revise` closes a fact rather than erasing it, so what it used to believe stays answerable. It runs only while idle, a live message **interrupts it immediately** (the pass resumes with everything it had already done), and it is bounded by [`memory.consolidationMaxSteps`](SETTINGS.md#memory-the-managers-long-term-memory) so a confused pass cannot go on forever.
