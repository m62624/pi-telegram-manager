...
- When a tool truncates its output, the full log — which otherwise exists only on the machine running Pi — is **attached to the chat as a file**, up to a byte cap you set ([`assistant.toolOutputMaxBytes`](SETTINGS.md#the-full-output-file-assistanttooloutputmaxbytes), 25 MiB by default, `0` to never attach). Cap it low on metered data.
