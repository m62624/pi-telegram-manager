...
- **Writing only happens in the background.** `manager_remember`, `manager_revise`, `manager_forget`, `manager_link` and `manager_unlink` are all consolidation-only — a live reply turn only *reads* the memory (the block above is assembled automatically, no tool call needed), so answering somebody is never held up by a write.
- **Memory consolidation.** When a chat has been quiet for [`manager.factConsolidationQuietMs`](SETTINGS.md#manager-business-manager-and-the-telegram-side-of-mixed) (default **30 min**), the model goes back over it with its memory tools in hand — `manager_recall` to look things up, `manager_remember`, `manager_revise` to replace what the conversation overturned, `manager_forget` to drop what was wrong, `manager_link`/`manager_unlink` to connect this person to a durable topic (or one topic to another) in the memory's own graph — and ends with `manager_done`. It **owns its own memory**: nothing is capped, evicted or expired on its behalf. A `revise` closes a fact rather than erasing it, so what it used to believe stays answerable. One pass covers **one chat**, for as many facts as that conversation holds — it ends when the model calls `manager_done`, not after a fixed number of writes — and the next queued chat starts on a later tick. It runs only while idle, a live message **interrupts it immediately** (the pass resumes with everything it had already done), and it is bounded by [`memory.consolidationMaxSteps`](SETTINGS.md#memory-the-managers-long-term-memory) so a confused pass cannot go on forever.
...
| --- | --- |
| `"assistant": { "thinkingPlaceholder": false }` | The animated `Thinking… / ▸ bash — npm test (4s)` trace. Already the default; it is beta. |
| `"assistant": { "draftPreviews": false }` | Streaming drafts — no live preview of the reply as it is written. |
| `"topics": { "enabled": false }` | Forum topics in the bot DM; everything goes to one flat chat instead. |
| `"assistant": { "toolActivity": false }` | The tool cards, and the code blocks inside them. |

Settings are read when a mode starts, so restart the mode after changing one (`/switch`, or the terminal command). Change one at a time.

If one of them fixes it, please [open an issue](https://github.com/m62624/pi-telegram-manager/issues) and say which.
